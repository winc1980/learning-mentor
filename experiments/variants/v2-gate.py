...
# ---- パッチ3：広い質問への規範例（番号付きリストの外に置く） ----
# 見出しをリストの途中に入れるとリストが分断されるので、リスト終了後に挿入する。
P3_ANCHOR = """### やってはいけない解説
"""

P3_ADDED = """### 「全体的に分からない」と言われたとき
...
def build():
    ...
    # 2026-09-07: このバリアントはマスターへ取り込み済み。
    # アンカー（元の文）はパッチ後も残っているため、再実行すると二重に当たる。
    if "材料の多さは、手順1を省く理由になりません" in body:
        raise SystemExit(
            "マスターに取り込み済みです（2026-09-07）。再生成すると二重適用になります。\n"
            "取り込み前の状態と比べたい場合は git で当該コミット以前を参照してください。")

    # パッチ1・2 はアンカーの直後に追記する。
    ...
    # パッチ3 はアンカーを置き換える（新しい節を、元の見出しの前に差し込む）。
    if P3_ANCHOR not in body:
        raise SystemExit("パッチ3 のアンカーが見つかりません。")
    body = body.replace(P3_ANCHOR, P3_ADDED, 1)
    return body
