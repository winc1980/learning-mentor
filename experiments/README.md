...
未コミットの状態で回した run は復元できず「不明」になる（いまは該当なし）。

### バリアントを使った run は、git を素直に引くと別の本文が返る

**003・005・006（v2-gate バリアント）がこれ。** バリアントは生成物なので、本体が更新されると
作り直されてコミットし直される。実際 `a398e78` でコミットされた `v2-gate.md` の本文は、
**同コミットの配布本文と完全に一致する**（v2-gate を本体に取り込んだあと、新しい本文から
作り直したもの）。003・005・006 がその前に読んだ本文とは別物なのに、引けば値が返るので
正しく見える。**これも静かに壊れる経路。**

そのため、実行時点でバリアントが git に無い run は、**当時の master と当時の生成器から
作り直して**ハッシュを取る（由来は「生成器で再生成（推定）」）。生成器は決定論的なので
当時の入力を揃えれば当時の出力が得られる。ただし生成後に手で直していれば分からないので、
これも推定であることに変わりはない。

この 3 つが読んだ本文は「v2-gate 前の本体（`48a436b`）に v2-gate のパッチを当てたもの」で、
**配布本文のどれとも一致しない。** `a398e78` の配布本文とは正規化後 20 行違う（項目の順序と
一文）。「v2-gate が入っているから現行相当」と読んで 008 以降と並べると、そこで解釈が壊れる。
3 つは互いには同一なので、006 の退行チェックは 003・005 と比較して読める。
